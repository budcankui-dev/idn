...
- 源终端：用户指定的训练数据来源终端名称（如"终端h1"、"终端h2"等），系统会自动查询其IP地址
- 目的终端：用户指定的训练结果目标终端名称（如"终端h3"、"终端h4"等），系统会自动查询其IP地址
- 开始时间：必须为有效日期时间格式 "YYYY-MM-DD HH:MM" 且不早于当前时间
- 期望运行时间：最短运行时间为{config.get_min_runtime_minutes()}分钟
- 训练完成时间：直接使用用户描述的数值（如"2小时"、"30分钟"），不需要转换为毫秒或精确时间，只需与"期望运行时间"数值上相同即可
