...
## 重要：参数命名规范（必须严格遵守）
- "训练轮数"、"轮数"、"n轮" 等表述 → 统一输出为 **"训练轮次"**
- "完成时间要求"、"完成时间"、"期望完成时间" → 统一输出为 **"训练完成时间"**
- 策略性描述（如"时间更快"、"尽快完成"）与训练完成时间是**不同概念**，不要混淆
3. 输出结果：
...
模型选择 yolov8，延迟不超过2秒，视频帧率30fps，分辨率1920x1080，源终端终端h1，目的终端终端h2。
...
## 示例:
### 用户输入：
我想部署视频AI推理业务，延迟不超过2秒，视频帧率30fps，分辨率1920x1080，源终端终端h1，目的终端终端h2。
