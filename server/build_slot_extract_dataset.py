    ...
    with open(input_file, 'r', encoding='utf-8') as fin, open(output_file, 'w', encoding='utf-8') as fout:
        for line in tqdm(fin, desc='Processing'):
            ...
            try:
                ...
                result = {"error": str(e)}
            record = {"prompt": prompt, "label": result}
            fout.write(json.dumps(record, ensure_ascii=False) + '\n')
            time.sleep(0.5)  # 防止接口压力过大

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='批量调用slot_extract接口生成数据集')
    parser.add_argument('--input', required=True, help='输入文本文件，每行一个prompt')
    parser.add_argument('--output', required=True, help='输出数据集文件（jsonl）')
